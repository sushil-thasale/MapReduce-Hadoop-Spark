...
/*
 * NoLockProcessing : the shared HashMap is not synchronized
 */
public class NoLockProcessing {

	SharedDataStructure sharedData;
	int maxCores = 8; // lscpu -> # of CPUs
	final String fileName = "output/no-lock.txt";
	long fileSize;
	long partitionSize;
	long startIndex, endIndex;
	long startTime, endTime;
	ArrayList<Long> executionTimes;
	ArrayList<NoLockThread> threads;

	public NoLockProcessing(ArrayList<String> fileData) {

		sharedData = new SharedDataStructure(fileData);
		fileSize = fileData.size();
		partitionSize = fileSize / maxCores;
		threads = new ArrayList<NoLockThread>();
	...
	public void execute(boolean addDelay) {
		...
		String programType = addDelay ? "No-Lock with delay"
				: "No-Lock without delay";
	...
	private void spawnThreads(boolean addDelay) {
		...
		for (int i = 0; i < maxCores; i++) {
			...
			threads.add(new NoLockThread(startIndex, endIndex, sharedData,
					addDelay));
		}
	}
